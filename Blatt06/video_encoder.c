...
static int diamond_search(const int16_t block[64],
    ...
    if (max_dist > 0) {
         ...
         * vertically neighbourinng the center.
         */
...
static int encode_video_cl(video *video, FILE *f, const ppp_image_info *info,
    ...
    cl_mem imageGPU, old_imageGPU, offsets_and_sizesGPU, sizeGPU, frameGPU;
    cl_mem motionsGPU;
    cl_int res;
...
/*
    if (compr) {
        cl_ulong nanos = get_event_end_nanos(reorderEvent) -
            get_event_start_nanos(reorderEvent);
