...
/*
 * Compute the (x,y)-position of the pixel in block 'block' and
 * offset 'offset' inside the block in an image with 'columns' columns'.
...
/*
 * Copy block number 'block_nr' from image (which is stored
 * row-wise) into 'block'. The pixel format is converted
 * from uint8_t to int16_t (i.e., 128 is subtracted from each
 * pixel value).
 */
void uint8_image_get_block(const uint8_t *image, int columns,
    ...
    for (int y=0; y<8; y++)
        for (int x=0; x<8; x++)
            block[8*y+x] = (int16_t)image[(yy+y)*columns + xx+x] - 128;
}

static void uint8_image_get_block8(const uint8_t *image, int columns,
                                   int block_nr, int8_t block[64]) {
    int yy, xx;
    block_to_row_col(block_nr, 0, columns, &yy, &xx);
    for (int y=0; y<8; y++)
        for (int x=0; x<8; x++)
            block[8*y+x] = (int8_t)((int16_t)image[(yy+y)*columns + xx+x]-128);
}
...
void qdct_block(const int16_t *f, int16_t *ff) {
     ...
     */
    for (int i=0; i<64; i++)
        ff[permut[i]] = lrintf(C[i] / quantization_factors[i]);
}
 ...
 *
 * Example:
 * The encoded sequence 0xC 0xB 0x2 0xA 0xF 0x1 0x2 0x0
 * is decoded as:
 *   0xC 0xB:     (0xB + 3) = 14
 ...
 * The input image 'image' is stored row-wise.
 * The macro blocks are extracted from 'image' before encoding.
...
void encode_frame(const uint8_t *image, int rows, int columns,
    ...
    int16_t currblock[64], qdctres[64];
    ...
    case PPP_IMGFMT_UNCOMPRESSED_BLOCKS:
        for (int b=0; b<n_blocks; b++)
            uint8_image_get_block8(image, columns, b,
                                   (int8_t *)&frame->data[64*b]);
        frame->length = pixels;
        break;
    case PPP_IMGFMT_UNCOMPRESSED_DCT:
        for (int b=0; b<n_blocks; b++) {
            uint8_image_get_block(image, columns, b, currblock);
            qdct_block(currblock, qdctres);
            for (int i=0; i<64; i++)
                frame->data[64*b+i] = (uint8_t)qdctres[i];
        }
        frame->length = pixels;
        break;
    case PPP_IMGFMT_COMPRESSED_DCT:
        ...
        for (int b=0; b<n_blocks; b++) {
            uint8_image_get_block(image, columns, b, currblock);
            frame->length += qdct_compress_block(currblock,
